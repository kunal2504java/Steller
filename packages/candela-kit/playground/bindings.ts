// Probatum v2 contract bindings for the candela-kit playground.
//
// REGENERATED from the live v2 contract (Task 4) — NOT copied from the spike's
// stale v1 copy (which still exposed `init` + the old contract id). Produced via:
//   stellar contract bindings typescript --network testnet \
//     --contract-id CDB6674ZKXTKH6M25CJY75EAOWXE44G7VH3ZI7VNGDZHD475KXDQR3YV \
//     --output-dir <tmp>
// then patched to `@stellar/stellar-sdk/minimal/*` exactly as the spike's copy
// was (the `/minimal` browser entry is a self-contained UMD bundle; mixing it
// with the full sdk copy makes XDR objects fail cross-copy type checks). The
// generator's `export * from "@stellar/stellar-sdk"` line is dropped to match
// the spike's copy (Client is the only export the playground consumes).
//
// v2 surface (vs the spike's v1 copy): no `init`, has `__constructor`,
// `NotInitialized` (#11), `ClaimCount` DataKey, `bump_batch`; `pause` now
// returns Result<void>. `register_issuer`'s signature is unchanged.
...
export const networks = {
  testnet: {
    ...
    contractId: "CDB6674ZKXTKH6M25CJY75EAOWXE44G7VH3ZI7VNGDZHD475KXDQR3YV",
  }
} as const
...
export type DataKey = {tag: "Admin", values: void} | {tag: "Paused", values: void} | {tag: "BatchSeq", values: void} | {tag: "Issuer", values: readonly [string]} | {tag: "Batch", values: readonly [u64]} | {tag: "RevokedLeaf", values: readonly [u64, Buffer]} | {tag: "Claim", values: readonly [u64, Buffer]} | {tag: "ClaimCount", values: void};
...
export interface Client {
  ...
  /**
   * Construct and simulate a bump_batch transaction. Returns an `AssembledTransaction` object which will have a `result` field containing the result of the simulation. If this transaction changes contract state, you will need to call `signAndSend()` on the returned object.
   */
  bump_batch: ({batch_id}: {batch_id: u64}, options?: MethodOptions) => Promise<AssembledTransaction<Result<void>>>
...
export class Client extends ContractClient {
  ...
  constructor(public readonly options: ContractClientOptions) {
    super(
      new ContractSpec([ "AAAAAQAAAAAAAAAAAAAABUJhdGNoAAAAAAAABgAAAAAAAAALYW5jaG9yZWRfYXQAAAAABgAAAAAAAAAFY291bnQAAAAAAAAEAAAAAAAAAAZpc3N1ZXIAAAAAABMAAAAAAAAABG1ldGEAAAPuAAAAIAAAAAAAAAAHcmV2b2tlZAAAAAABAAAAAAAAAARyb290AAAD7gAAACA=",
        "",
        "",
        ...
        "AAAABQAAAE1FbWl0dGVkIHdoZW4gcGF1c2Ugc3RhdGUgaXMgdG9nZ2xlZC4KVG9waWNzOiAoYHBhdXNlX3RvZ2dsZWRgKS4gRGF0YTogcGF1c2VkLgAAAAAAAAAAAAAMUGF1c2VUb2dnbGVkAAAAAQAAAA1wYXVzZV90b2dnbGVkAAAAAAAAAQAAAAAAAAAGcGF1c2VkAAAAAAABAAAAAAAAAAA=",
        ...
        "AAAAAAAAAAAAAAAFcGF1c2UAAAAAAAABAAAAAAAAAAZwYXVzZWQAAAAAAAEAAAABAAAD6QAAA+0AAAAAAAAAAw==",
        ...
        "AAAAAAAAAAAAAAAKYnVtcF9iYXRjaAAAAAAAAQAAAAAAAAAIYmF0Y2hfaWQAAAAGAAAAAQAAA+kAAAPtAAAAAAAAAAM=",
        ...
        "AAAAAAAAAEBSdW5zIGF0b21pY2FsbHkgYXQgZGVwbG95IHRpbWUg4oCUIG5vIGluaXQgZnJvbnQtcnVubmluZyB3aW5kb3cuAAAADV9fY29uc3RydWN0b3IAAAAAAAABAAAAAAAAAAVhZG1pbgAAAAAAABMAAAAA",
  ...
  public readonly fromJSON = {
    claim: this.txFromJSON<Result<void>>,
        ...
        bump_batch: this.txFromJSON<Result<void>>,
